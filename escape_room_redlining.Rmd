...
[2020 Historic Redlining Scores Dataset (OpenICPSR)](https://www.openicpsr.org/openicpsr/project/141121/version/V2/view?path=/openicpsr/141121/fcr:versions/V2/HRS2020B.sav&type=file).
...
Download the dataset and save it as `HRS2020B.sav` in the `data` folder of your project.

### Challenge:

```{r room2_question1}
question("What is the size of the downloaded file in MB?",
  answer("Approximately 871 KB.", correct = TRUE),
  answer("Approximately 3240 KB.", correct = FALSE),
  allow_retry = TRUE
)
```
...
hrs_data <- read.spss("HRS2020B.sav", to.data.frame = TRUE)
...
average_hrs <- hrs_data %>%
  dplyr::filter(
    ...
    METRO_NAME == "Chicago-Naperville-Elgin, IL-I"    
  ) %>%
